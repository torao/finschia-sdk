...
func checkTrustedHeader(header *Header, consState *ConsensusState) error {
	ocTrustedValidators, err := tmtypes.ValidatorSetFromProto(header.TrustedValidators)
	if err != nil {
	...
	tvalHash := ocTrustedValidators.Hash()
...
func checkValidity(
	...
	ocTrustedVoters, err := tmtypes.VoterSetFromProto(header.TrustedVoters)
	if err != nil {
		return sdkerrors.Wrap(err, "trusted validator set in not tendermint validator set type")
	}

	ocSignedHeader, err := tmtypes.SignedHeaderFromProto(header.SignedHeader)
	if err != nil {
		return sdkerrors.Wrap(err, "signed header in not tendermint signed header type")
	}

	ocVoterSet, err := tmtypes.VoterSetFromProto(header.VoterSet)
	if err != nil {
		return sdkerrors.Wrap(err, "voter set in not tendermint voter set type")
	}
	...
	err = light.VerifyWithVoterSet(
		&signedHeader,
		ocTrustedVoters, ocSignedHeader, ocVoterSet,
