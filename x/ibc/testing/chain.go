...
import (
	...
	ocprotoversion "github.com/tendermint/tendermint/proto/tendermint/version"
...
func NewTestValidator(pubkey crypto.PubKey, stakingPower int64) *tmtypes.Validator {
	val := tmtypes.NewValidator(pubkey, stakingPower)
...
func NewTestChain(t *testing.T, chainID string) *TestChain {
	...
	chain := &TestChain{
		...
		Codec:         app.AppCodec(),
		Vals:          valSet,
		Voters:        tmtypes.WrapValidatorsToVoterSet(valSet.Validators),
		Signers:       signers,
...
func (chain *TestChain) GetValsAtHeight(height int64) (*tmtypes.ValidatorSet, bool) {
	...
	return tmtypes.NewValidatorSet(ocValidators), true
}
...
// CreateOCClient will construct and execute a 99-tendermint MsgCreateClient. A counterparty
...
// UpdateOCClient will construct and execute a 99-tendermint MsgUpdateClient. The counterparty
...
// ConstructUpdateOCClientHeader will construct a valid 99-tendermint Header to update the
...
func (chain *TestChain) ConstructUpdateOCClientHeader(counterparty *TestChain, clientID string) (*ibctmtypes.Header, error) {
	...
	var (
		ocTrustedVals   *tmtypes.ValidatorSet
		ocTrustedVoters *tmtypes.VoterSet
		ok              bool
	)
...
func (chain *TestChain) CreateOCClientHeader(chainID string, blockHeight int64, trustedHeight clienttypes.Height, timestamp time.Time, ocValSet, ocTrustedVals *tmtypes.ValidatorSet, ocVoterSet, ocTrustedVoterSet *tmtypes.VoterSet, signers []tmtypes.PrivValidator) *ibctmtypes.Header {
	...
	voteSet := tmtypes.NewVoteSet(chainID, blockHeight, 1, tmproto.PrecommitType, ocVoterSet)
	...
	var trustedVoters *tmproto.VoterSet
